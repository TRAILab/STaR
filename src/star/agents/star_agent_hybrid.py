...
from star.utils.util import file_to_string
from star.utils.utils import print_to_cot_log
from star.agents.functions_wrapper import FunctionsWrapper

from star.memory.memory import Memory

from star.agents.agent import Agent, AgentOutput
...
class STaRAgent_VANILA(Agent):
    ...
    def __init__(self, llm_type='gpt-4o', vlm_type='gpt-4o', num_ctx=8192, temperature=0):

        # Wrapper that handles everything
        llm = self.llm_selector(llm_type, temperature, num_ctx)
        if vlm_type != "":
            vlm = self.llm_selector(vlm_type, temperature, num_ctx)
            self.multimodal = FunctionsWrapper(vlm)
        chat = FunctionsWrapper(llm)

        self.cached = [] # cache the tool calls for VLMs
    ...
    def llm_selector(self, llm_type, temperature, num_ctx):
        ...
        elif llm_type == 'command-r': #  or 'gpt-oss':
    ...
    def set_memory(self, memory: Memory, scene_graph=None, dataset_start_timestamp=None, sbert_model=None, test_num=None, args=None):
        self.memory = memory
    ...
    def create_tools(self, memory):
        ...
        class NearestObjectInput(BaseModel):
            ...
            object_positions: list = Field(
                description=(
                    "A list of candidate object positions as numeric coordinate lists only, "
                    "for example [[x1, y1, z1], [x2, y2, z2]]. Do not include comments, "
                    "labels, timestamps, object names, or any non-numeric values inside this list."
                )
            )
    ...
    def agent(self, state):
        ...
        prompt += (
            "\n\nWhen calling tools, provide strictly valid JSON tool arguments. "
            "Do not include comments such as //, labels, timestamps, or explanations "
            "inside JSON arrays or objects. Put reasoning only in natural-language fields, "
            "not inside tool_input values."
        )
        ...
        if ('gpt-4' not in self.llm_type) and ('nim' not in self.llm_type) and ('o4' not in self.llm_type):
            ...
            for i in range(len(messages)):
                if type(messages[i]) == ToolMessage:
                    cached_call = ToolMessage(content=messages[i].content, id=messages[i].id, tool_call_id=messages[i].tool_call_id)
                    self.cached.append((i, cached_call)) if self.vlm_type != "" else None
        ...
        # print(colored(f"Debug response: 'question': {question}, 'chat_history': {messages[:]}", "red", attrs=["bold"]))
    ...
    def generate_with_timestamp(self, state):
        ...
        messages = state["messages"]
        for cached_call in self.cached:
            print("Poping cache")
            idx, tool_call = cached_call
            messages[idx] = tool_call
        ...
        if multimodal_inputs:
            ...
            model = gen_prompt | self.multimodal
            response = model.invoke({"question": question, "chat_history": messages[1:]})
            ...
            model = gen_prompt | self.multimodal if self.vlm_type != "" else gen_prompt | self.chat
            response = model.invoke({"question": question, "chat_history": messages[1:]})
        # Post-process output
