...
class ClusterIB():
    ...
    def setup_py_x(self, region_features, task_features):
        # Build the information source for IB:
        # - p(x): prior over objects X
        # - p(y|x): how strongly each object supports each task Y
        # - p(y): prior over tasks
        self.px, self.py_x, self.py = self.compute_initial_probabilities(
            region_features, task_features)
        self.n = len(self.px)
        # I(X;Y): task information available before any compression.
    ...
    def compute_initial_probabilities(self, region_features, task_features):
        """
        Construct p(y|x) from embedding similarities.

        Row layout of py_x:
        - row 0: null-task / background bucket
        - rows 1..T: real task/query prompts

        For a single text query, each object x gets a two-way distribution:
        - p(null | x)
        - p(query | x)
        """
        ...
        px = np.ones(n) / n
        py = np.ones(m) / m
        # Normalize each object column so sum_y p(y|x) = 1.
    ...
    def find_clusters(self):
        """
        Agglomerative IB over graph-valid merges.

        Starts from one cluster per object and repeatedly merges the edge with
        the lowest merge cost:

            w(i, j) = (p(c_i) + p(c_j)) * JS( p(y|c_i), p(y|c_j) )

        The algorithm stops when the normalized task-information loss is too
        large:

            dI(C;Y) = I_prev(C;Y) - I_new(C;Y)
            delta   = dI(C;Y) / I(X;Y)

        Only edges present in the graph are eligible, so clustering is both
        semantic and spatial/topological.
        """

        # Initialize p(c|x) as identity: every object starts in its own cluster.
        pc_x = np.eye(self.n)
        # p(c) = sum_x p(c|x) p(x)
        pc = pc_x @ self.px

        # Because clusters are singleton at the start, p(y|c) is inherited from p(y|x).
        py_c = self.py_x * self.px @ np.transpose(pc_x) / pc

        # I(C;Y) before any merges.
        ...
        while True:
            ...
            # Merge cluster masses: p(c_new) = p(c_i) + p(c_j).
            pc_temp[min_edge[0]] = pc[min_edge[0]] + pc[min_edge[1]]
            pc_temp[min_edge[1]] = 0

            # Merge the corresponding rows in p(c|x).
            pc_x_temp[min_edge[0], :] = pc_x[min_edge[0], :] + \
                pc_x[min_edge[1], :]
            pc_x_temp[min_edge[1], :] = 0

            # Task information retained after the proposed merge.
    ...
    def compute_edge_weight(self, pc, py_c, i, j):
        # Weighted Jensen-Shannon merge cost:
        # merge if the two clusters have similar p(y|c), avoid merging when they
        # represent meaningfully different task semantics.
