...
from .helpers import (
    init_scenegraph,
    # prepare_mos_model,  # 4DMOS dynamic filtering is disabled for now.
    load_background_objects,
    save_scene_graph,
    iter_by_event_pcd
)

# 4DMOS dynamic filtering is disabled for now. Historical point-cloud
# accumulation still runs, but no 4DMOS import path is required.
# mos4d_paths = [
#     os.environ.get("STAR_4DMOS_PATH"),
#     "/workspace/third_parties/4DMOS/src/mos4d",
#     "/workspace/third_parties/4DMOS/src",
# ]
# for mos4d_path in mos4d_paths:
#     if mos4d_path and os.path.isdir(mos4d_path) and mos4d_path not in sys.path:
#         sys.path.append(mos4d_path)
...
def create_2d_map_point(detected_objects, bg_list, pose, prev_occupancy_map, prev_map_origin, pre_detection_list, pre_bg_list, resolution=0.1):
    ...
    drive_area = drivable_indices[-1] if drivable_indices else None
    if terrain_pcd:
        terrain_map = build_terrain_map(terrain_map, terrain_pcd, map_origin, resolution)
    if drive_area is not None:
...
def backward_projection_RGB(road_pcd_list, object_pcd_list, trans_pose, calib):
    ...
    road_cam = camera_projection_matrix.dot(lidar_to_camera_matrix).dot(road_pcd_homo_inv)
    object_cam = camera_projection_matrix.dot(lidar_to_camera_matrix).dot(object_pcd_homo_inv)
...
def create_SoM_keypts(img, sematic_info, drivable_indices, interval=20):
    '''
...
def shift_objects_to_positive_plane(detect_object, resolution=0.25, map_origin=(0, 0, 0), map_size=(400, 800)):
    ...
    for i, obj in enumerate(detect_object, start=0):
        # if objects[i]['image_idx'][-1] == idx:
        pcl = detect_object[i]["pcd"]
        bbox = detect_object[i]['bbox']
        img_bbx = detect_object[i]['img_bbox']
        rounded_center = np.round(bbox.get_center(), 3)
        rounded_extent = np.round(bbox.get_extent(), 3)
...
def run_scenegraph_generation(
    ...
    if prepare_event is not None:
        prepare_event.set()
    # 4DMOS dynamic filtering is disabled for now.
    if "filter_dynamic" in cfg:
    ...
    enable_timing_logs = bool(getattr(cfg, "enable_timing_logs", True))
    ...
    try:
        data_iter = iterator(data_source, loop_event, cfg)
        while True:
            frame_total_start = time.perf_counter()
            fetch_start = time.perf_counter()
            try:
                data_in_window = next(data_iter)
            except StopIteration:
                break
            fetch_dt = time.perf_counter() - fetch_start
            queue_depth_after_fetch = safe_qsize(data_source)
            image, pc, pose, his_pcs, his_poses, timestamp = data_in_window

            dam_start = time.perf_counter()
            file_names = idx
            ...
            dam_dt = time.perf_counter() - dam_start
            dam_timing = getattr(mask_generator, "last_timing", {})
            # Accumulate the point cloud
            accumulate_start = time.perf_counter()
            if his_pcs is not None:
                pc = accumulate_pc(cfg, mos_model, pc, pose, his_pcs, his_poses) # Accumulate historical point clouds
            pc = pc[pc[:, 2] <= 4.0] # Filter out points that are too high
            # Filter out points that are too far
            if cfg.filter_dis:
                pc = distance_filter(cfg.max_depth, pc)
            accumulate_dt = time.perf_counter() - accumulate_start

            project_start = time.perf_counter()
            pro_point_new, pixels_new, z_new = project(pc, image, calib, cfg)
            project_dt = time.perf_counter() - project_start
            ...
            # Plot the image
                back_project_start = time.perf_counter()
                import matplotlib.pyplot as plt
                ...
                back_project_dt = time.perf_counter() - back_project_start
            ...
            # if print_scenegraph_captions and (idx % timing_log_every_n == 0):
            #     for det_idx, detected_object in enumerate(detection_list):
            #         print(
            #             f"[scenegraph-caption] idx={idx} det={det_idx}: "
            #             f"{detected_object.get('caption', '')}",
            #             flush=True,
            #         )

            bg_merge_start = time.perf_counter()
            if len(bg_list) > 0:
            ...
            bg_merge_dt = time.perf_counter() - bg_merge_start
            ...
            if len(detection_list) == 0:
                ...
                poses.append(pose)
                total_dt = time.perf_counter() - frame_total_start
                if enable_timing_logs and (idx % timing_log_every_n == 0):
                    print(
                        colored(
                            "[timing][scenegraph] "
                            f"idx={idx} timestamp={timestamp:.3f} queue_after_fetch={queue_depth_after_fetch} "
                            f"fetch={fetch_dt:.3f}s dam={dam_dt:.3f}s "
                            f"dam_tag={dam_timing.get('tag', 0.0):.3f}s "
                            f"dam_dino={dam_timing.get('dino', 0.0):.3f}s "
                            f"dam_nms={dam_timing.get('nms', 0.0):.3f}s "
                            f"dam_mask_caption={dam_timing.get('mask_caption', 0.0):.3f}s "
                            f"dam_vis={dam_timing.get('vis', 0.0):.3f}s "
                            f"accumulate={accumulate_dt:.3f}s project={project_dt:.3f}s "
                            f"back_project={back_project_dt:.3f}s detect={detect_dt:.3f}s "
                            f"bg_merge={bg_merge_dt:.3f}s sim=0.000s merge=0.000s annotate=0.000s "
                            f"total={total_dt:.3f}s detections=0 dam_results={dam_timing.get('results', 'NA')} "
                            f"dam_mode={dam_timing.get('mode', 'NA')}",
                            "white",
                            attrs=["dark"],
                        ),
                        flush=True,
                ...
                if enable_timing_logs and (idx % timing_log_every_n == 0):
                    print(
                        colored(
                            ...
                            f"back_project={back_project_dt:.3f}s detect={detect_dt:.3f}s "
                            f"bg_merge={bg_merge_dt:.3f}s sim=0.000s merge=0.000s annotate=0.000s "
                            f"total={total_dt:.3f}s detections={len(detection_list)} "
                            f"dam_results={dam_timing.get('results', 'NA')} "
                            f"dam_mode={dam_timing.get('mode', 'NA')} bootstrap=1",
                            "white",
                            attrs=["dark"],
                        ),
                        flush=True,
            ...
            merge_dt = time.perf_counter() - merge_start
            ...
            annotate_dt = time.perf_counter() - annotate_start
            ...
            if enable_timing_logs and (idx % timing_log_every_n == 0):
                print(
                    colored(
                        ...
                        f"accumulate={accumulate_dt:.3f}s project={project_dt:.3f}s "
                        f"back_project={back_project_dt:.3f}s detect={detect_dt:.3f}s "
                        f"bg_merge={bg_merge_dt:.3f}s sim={sim_dt:.3f}s merge={merge_dt:.3f}s "
                        f"annotate={annotate_dt:.3f}s total={total_dt:.3f}s "
                        f"detections={len(detection_list)} objects={len(objects)} "
                        f"dam_results={dam_timing.get('results', 'NA')} "
                        f"dam_mode={dam_timing.get('mode', 'NA')}",
                        "white",
                        attrs=["dark"],
        ...
        # The online memory artifact is complete once full_pcd.pkl.gz is saved.
        # Background cleanup is post-save housekeeping and should not make a
        # successful run look failed during shutdown.
        try:
            ...
            print(f"[memory][warning] Post-save background cleanup skipped: {exc}")
